...
public class Code_02_InsertSort {
	...
	/*
	* 1.首先判断数组是不是为空，或者数组只有一个数的话，就直接返回，因为数组中只有一个数就不需要排序
	* 2.外层循环先从第二个数开始遍历i,
	* 3.内层循环先从上一个循环的j=i-1(这个其实是上一轮中已经排好序的那个)开始，j要和后面的
	* 	j+1（这个是需要比交的那个）进行比较，
	* 	如果已经排好的那个大于后面需要排序的那个，就需要交换，否则，跳出内层循环，然后j继续跟前面的比较，然后外层循环继续下一个
	*
	* */
